---
title:  |
  | 16S Sequel IIe analysis report
author: 
...
# R --slave -e 'rmarkdown::render("/path_to/DADA2_pacbio-reads.Rmd", "pdf_document")'
...
Analyze HiGFi read data obtained from sequencing multiple 16S amplicon samples, quantify the organisms present in each sample, identify them by comparing to a taxonomy reference, and annotate the obtained **Amplicon Specific Variants (ASV)**, finally, produce quality control and exploratory plots to document the process and provide useful data to the customer.
...
# plot fastq qualities and lengths for the first 8 samples
rqp <- suppressWarnings(plotQualityProfile(fnFs[1:8],
                        n=5e+5,
                        aggregate=FALSE))
...
* trim primer sequences at both ends (`r nchar(forwardprimer)` and `r nchar(reverseprimer)` nucleotides)
* discard reads shorter than `r minLen` or larger than `r maxLen` nucleotides and too far from the expected amplicon size
...
Note: reads shorter than `r minLen`or larger than `r maxLen` nucleotides are removed by default.
...
```{r Filter and trim}
sample.names <- gsub(".f[astq]+.gz", "", basename(fnFs))

filtFs <- file.path(outpath, "filtered_reads", paste0(sample.names, "_filt.fq.gz"))
names(filtFs) <- sample.names
trimLeft <- nchar(forwardprimer)
trimRight <- nchar(reverseprimer)
...
fqp <- suppressWarnings(plotQualityProfile(filtFs[1:8],
                                           n=5e+5,
                                           aggregate=FALSE))
...
Figure 3: Error plots for the filtered reads
...
The inferred **Amplicon Specific Variants** (ASV) are stored into a sequence table analogous to an **Operational taxonomic unit (OTU)** table.

Table: Lengths and Count of the ASV sequences 
